...
function getPasswordStrength(value) {
  ...
  if (score < 75) return { value: score, label: 'Moderate', color: 'bg-amber-500' };
  ...
  const validations = useMemo(() => {
    return {
      name: {
        ...
        message: form.name.trim().length >= 2 ? 'Looks good' : 'Use at least 2 characters',
      },
      email: {
        valid: EMAIL_PATTERN.test(form.email.trim()),
        message: EMAIL_PATTERN.test(form.email.trim()) ? 'Valid email format' : 'Enter a valid email',
      },
      password: {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-20 bg-background relative overflow-hidden">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-on-surface-variant">Join thousands of curious minds.</p>
        </div>
        ...
        <div className="glass-card rounded-[2rem] p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div className="space-y-1.5">
              ...
              <div className={`group relative flex items-center rounded-xl border bg-surface-container/80 transition-all duration-300 ${focused === 'name' ? 'border-primary shadow-[0_0_0_3px_hsl(var(--primary)/0.15)]' : 'border-outline-variant/60'}`}>
                <motion.div
                  animate={{ scale: focused === 'name' ? 1.12 : 1, color: focused === 'name' ? 'hsl(var(--primary))' : 'hsl(var(--on-surface-variant))' }}
                ...
                <input
                  ...
                  onBlur={() => { setFocused(''); setFieldTouched('name'); }}
                  placeholder=" "
                  autoComplete="name"
                ...
                <div className="pr-3"><FieldIndicator field="name" /></div>
              </div>
              <p id="register-name-hint" className="text-xs text-on-surface-variant">{validations.name.message}</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p id="register-email-hint" className="text-xs text-on-surface-variant">{validations.email.message}</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p id="register-password-hint" className="text-xs text-on-surface-variant">{validations.password.message}</p>
              <div className="space-y-1.5" aria-live="polite">
                <div className="flex items-center justify-between text-xs text-on-surface-variant">
                  <span>Password strength</span>
                  <span id="register-strength-hint" className="font-semibold">{strength.label}</span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button type="button" variant="outline" onClick={() => socialRegister('Google')} className="w-full">
                <Globe size={16} /> Google
              </Button>
              <Button type="button" variant="outline" onClick={() => socialRegister('GitHub')} className="w-full">
